...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  if (request.params.name === "search_duplicate_issues") {
    const { repo, query } = request.params.arguments as { repo: string, query: string };
    try {
  ...
  if (request.params.name === "scan_repo_duplicates") {
    const { repo } = request.params.arguments as { repo: string };
    try {
  ...
  if (request.params.name === "close_duplicate_issue") {
    const { repo, issue_number, duplicate_of_number } = request.params.arguments as { repo: string, issue_number: number, duplicate_of_number: number };
    try {
